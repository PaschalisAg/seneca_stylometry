...
raw.corpus <- load.corpus(files = "all", 
                          corpus.dir = "corpora/corpus_pca_bct/",
                          encoding = "UTF-8")
...
# apply PCA cov matrix to the given corpus (i.e., Seneca, Lucan, Statius)
results.pca.4grams.cor <- stylo(frequencies = freqs.4grams.table,
                                analysis.type = "PCR",
                                mfw.min = 100,
                                mfw.max = 2000,
                                incrementation = 100,
                                distance.measure = "wurzburg",
                                custom.graph.title = "Seneca against Lucan, and Statius",
                                write.pdf.file = T,
                                gui = T)
