...
raw.corpus <- load.corpus(
  files = "all",
  corpus.dir = "validation_imp_corpus/",
  encoding = "UTF-8"
)
```
...
for (n in 1:nrow(data)) {
  ...
  # apply the imposters method to compare the test text against the reference set
  # and extract the maximum result for the current test text
  result <- max(summary(imposters(reference.set = reference.set,
                                   test = test,
                                   iterations = 100,
                                   distance = 'wurzburg')))
  ...
  results.imposters <- rbind(results.imposters,
                             data.frame(document = rownames(data)[n],
                                        result = result))
}
...
write_xlsx(results.imposters, "../validation_imposters/results_imposters_valid_cosine.xlsx")
```
